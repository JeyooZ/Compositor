    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if session.tool == .eyedropper {
                HStack(spacing: 16) {
                    Text(L10n.string("Eyedropper")).font(ToolHeaderStyle.titleFont)
                    Toggle(L10n.string("Sample Ring"), isOn: $session.showsSampleRing).toggleStyle(.checkbox)
                    Spacer()
            ...
            if session.tool == .idle {
                HStack(spacing: 16) {
                    Text(L10n.string("Select a tool")).font(ToolHeaderStyle.titleFont)
                    Spacer()
                }.padding(.horizontal, 18).toolHeaderBar()
                Divider()
            }
        ...
        .toolbar {
            ToolbarItem(placement: .navigation) {
                Button { requestNewCanvas() } label: { Label(L10n.string("New canvas"), systemImage: "plus") }
                    .help(L10n.string("New canvas (⌘N)")).accessibilityIdentifier("newCanvasToolbar")
            ...
            ToolbarItemGroup(placement: .primaryAction) {
                Button(L10n.string("Fit")) { session.fit() }.help(L10n.string("Fit canvas in window (⌘0)"))
                    .accessibilityIdentifier("fitCanvas").disabled(session.document == nil)
                Button("100%") { session.zoom(to: 1) }.help(L10n.string("Actual pixels (⌘1)"))
            ...
            ToolbarItemGroup(placement: .primaryAction) {
                ...
                }.help(L10n.string("Zoom in (⌘+)")).disabled(session.document == nil)
                Button { session.zoom(to: session.viewport.zoom / 1.25) } label: {
                    Image(systemName: "minus.magnifyingglass")
                }.help(L10n.string("Zoom out (⌘−)")).disabled(session.document == nil)
            }
        }
        .onChange(of: session.levels == nil) { _, closed in
            if closed { levelsPanel.close() }
                ...
                levelsPanel.show(title: L10n.string("Levels"), content: LevelsSheet(session: session))
            }
        }
        .onChange(of: session.hueSaturation == nil) { _, closed in
            if closed { adjustmentPanel.close() }
                ...
                adjustmentPanel.show(title: L10n.string("Hue/Saturation"), content: HueSaturationSheet(session: session))
            }
        }
        .onChange(of: session.filterEdit == nil) { _, closed in
            if closed { filterPanel.close() }
                ...
                filterPanel.show(title: session.filterEdit.map { L10n.string($0.kind.rawValue) } ?? L10n.string("Filter"), content: FilterSheet(session: session))
            }
        }
        ...
        .alert(L10n.string("Import couldn’t finish"), isPresented: Binding(
            get: { session.importError != nil }, set: { if !$0 { session.importError = nil } })) {
                Button(L10n.string("OK"), role: .cancel) { session.importError = nil }
            } message: { Text(session.importError ?? "") }
        .alert(L10n.string("Couldn’t paint"), isPresented: Binding(get: { session.brushError != nil },
            set: { if !$0 { session.brushError = nil } })) {
                Button(L10n.string("OK")) { session.brushError = nil }
            } message: { Text(session.brushError ?? "") }
        .alert(L10n.string("Couldn’t crop"), isPresented: Binding(get: { session.cropError != nil },
            set: { if !$0 { session.cropError = nil } })) {
                Button(L10n.string("OK")) { session.cropError = nil }
    ...
    private var statusBar: some View {
        HStack(spacing: 16) {
            if let document = session.document {
                ...
                Text(L10n.string("sRGB · Transparent"))
            } else { Text(L10n.string("Ready when you are")) }
            Spacer()
            if session.showsBusy {
                ProgressView().controlSize(.mini)
                Text(L10n.string("Working…"))
    ...
    var body: some View {
        Divider().overlay {
            Color.clear.frame(width: 8).contentShape(Rectangle())
                ...
                .help(L10n.string("Drag to resize the panel"))
        }
    }
}
